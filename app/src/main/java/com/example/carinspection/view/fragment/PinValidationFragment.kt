...
import com.example.carinspection.databinding.FragmentPinBinding
 ...
 */
class PinValidationFragment : Fragment(), View.OnClickListener {
    ...
    private var fragmentPinBinding: FragmentPinBinding? = null
    ...
    override fun onCreateView(
        ...
        fragmentPinBinding = FragmentPinBinding.inflate(inflater, container, false)
        initViews()
        setListeners()
        viewdata = fragmentPinBinding?.root
        return viewdata
    }
    ...
    private fun setListeners() {
        fragmentPinBinding?.nextBtn?.setOnClickListener(this)
    }
    ...
    private fun goToNext()  {
        if(SharedPrefrenceHelper.getScreenPin(context).equals(getPin))
        {
            goToListOfInspection()
        }
    }

    private fun goToListOfInspection() {
    ...
    private fun checkValidation(): Boolean {
        ...
        getPin = fragmentPinBinding?.loginPin?.text.toString()
         if (getPin == "" || getPin.length == 0) {
            Toast.makeText(context,"PLease enter pin",Toast.LENGTH_LONG).show()
            isValid = false
             }
